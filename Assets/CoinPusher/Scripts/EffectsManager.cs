...
public class EffectsManager : MonoBehaviour {
	...
	public void runEffect(CoinEffect.Effect typeOfCoin, int coinValue = 0, PrizeData prizeData = null)
	{
		// Which effect are we?
		switch( typeOfCoin )
		{
		...
		case CoinEffect.Effect.CollectableSushi:
			this.triggerCollectableSushi(coinValue, CoinEffect.Effect.CollectableSushi, prizeData);
			break;

		case CoinEffect.Effect.DiaCoin:
			this.triggerDiaCoinEffect();
			playRewardSFX();
			break;
		}
	}
	...
	public void triggerDiaCoinEffect()
	{
		// Drop 10 coins
		coinSpawner.coinAttackSpawner(10);

		// Show the popup about the coin attack (reusing coinAttackPopup for now)
		showPopupText(coinAttackPopup);
	}
	...
	public void triggerCollectableSushi(int amount, CoinEffect.Effect effect, PrizeData prizeData)
	{
		// showPopupText(niceJobPopup);
		...
		_achievementManager.SaveProgress( prizeId );
